...
import hashlib
from agentic_ai import agentic_ai_chart_tab, track_agentic_ai_usage
import time
import json
import random
...
with st.sidebar:
    ...
    # Option 1: For TESTING ONLY - Force Pro role
    # Add this at the beginning of your main.py, after session state initialization
    if 'user_role' not in st.session_state:
        st.session_state.user_role = "Pro"  # Force Pro role for testing
    
    # Or temporarily override for testing:
    # st.session_state.user_role = "Pro"  # Uncomment this line for testing
    
    # Option 2: Add a developer mode toggle in the sidebar
    # Add this before the Analytics Mode section
    ...
    # Option 3: Check what role you actually have
    # Add this debug info to see current state
    st.sidebar.markdown("---")
    ...
    # Option 4: Modify the condition to allow testing
    # Replace the Analytics Mode section with this for testing:
    ...
    if TESTING_MODE or st.session_state.get("user_role") in ["Pro", "Enterprise"]:
        ...
        if st.session_state.agentic_ai_enabled:
            st.info("🤖 Agentic AI enabled! Unlock advanced insights.")
            
            # Chart generation mode checkbox
            st.session_state.use_code_based_charts = st.checkbox(
                "Use Code-Based Charts (Recommended)",
                value=st.session_state.use_code_based_charts,
                help="✓ Checked: Generate charts using verified Python code (reduces AI hallucination)\n✗ Unchecked: Fully AI-generated charts with rich insights"
            )
            
            if st.session_state.use_code_based_charts:
                st.caption("📊 Using code-verified charts")
            else:
                st.caption("🎨 Using AI-rendered charts")
        else:
